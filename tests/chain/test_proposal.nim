...
suite "chain/proposal":
  test "selectTxsForProposal lazily caches byteSize and execGas":
    ...
    check m.txs[tx1.hash].byteSize.get == encodeSignedMantleTx(tx1.signedTx).len
  ...
  test "selectProposalReferences enforces maxBytes budget":
    ...
    let tx1Bytes = encodeSignedMantleTx(tx1.signedTx).len
    let tx2Bytes = encodeSignedMantleTx(tx2.signedTx).len
    ...
    let (_, countAll) = m.selectProposalReferences(
      state, testLedgerConfig, SlotNumber(10),
  ...
  test "selectProposalReferences stops search after MaxConsecutiveCandidateMisses":
    ...
    let initialTxBytes = encodeSignedMantleTx(initialTx.signedTx).len
    ...
    let tinyTxBytes = encodeSignedMantleTx(tinyTx.signedTx).len
  ...
  test "selectTxsForProposal retains transactions with transient InvalidParent in mempool":
    ...
    testTransientRetention(ValidSignedMantleTx(signedTx: txTransient, hash: txHash))
