...
func utxos*(s: UtxoStore): lent HashTrieMap[NoteId, tuple[utxo: Utxo, leafIndex: int]] =
...
func insert*(
    s: UtxoStore, id: NoteId, utxo: sink Utxo
  ...
  let (newTree, leafIndex) = s.tree.insert(id)
  (UtxoStore(utxos: s.utxos.insert(id, (utxo, leafIndex)), tree: newTree), leafIndex)
